    ...
    return (
      <div className="min-h-screen bg-n-0 flex flex-col px-6 pt-12 pb-10">
        ...
        <div className="flex justify-center mb-6">
          <div className="w-16 h-16 rounded-full bg-pri-50 border border-pri-100 flex items-center justify-center">
            <Mail size={28} className="text-pri-500" />
          </div>
        </div>
        ...
        <p className="font-body text-sm text-n-400 text-center mb-8">
          We sent a code to{" "}
          <span className="font-semibold text-n-700">{email}</span>
        </p>
        ...
        <form onSubmit={handleOtpSubmit} className="flex flex-col items-center gap-6">
          <input
            ...
            className={`w-full text-center font-display text-3xl font-bold tracking-[0.4em] py-4 rounded-xl border-2 bg-n-50 text-n-900 placeholder:text-n-200 focus:outline-none transition-colors ${
              otp.length === 6 ? "border-pri-400" : "border-n-100"
            }`}
          />
        ...
        <p className="font-body text-sm text-n-400 text-center mt-8">
          Didn&apos;t receive it?{" "}
          <button className="text-pri-600 font-medium">Resend</button>
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-n-0 flex flex-col px-6 pt-12 pb-10">
      ...
      <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
        ...
        <div className="relative">
          <User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-n-300" />
          <input
            type="text"
            ...
            {[1, 2, 3, 4].map((bar) => (
              <div
                ...
                className={`h-1 flex-1 rounded-full transition-colors ${
                  bar <= strengthBars ? strengthColor : "bg-n-100"
                }`}
              />
            ))}
          </div>
        ...
        >
          ...
          <span className="font-body text-xs text-n-400 leading-relaxed">
            I agree to the{" "}
            <span className="text-pri-600 font-medium">Terms of Service</span>{" "}
            and{" "}
            <span className="text-pri-600 font-medium">Privacy Policy</span>
          </span>
        </button>
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl font-body font-semibold text-base transition-all active:scale-95 mt-2 ${
            isFilled
              ? "bg-gradient-to-r from-pri-500 to-pri-600 text-n-0 shadow-lg shadow-pri-900/20"
              : "bg-n-100 text-n-300 cursor-not-allowed"
          }`}
